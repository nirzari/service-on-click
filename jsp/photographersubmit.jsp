...
<%
	...
	String c = request.getParameter("venue");    
	String d[] = request.getParameterValues("format");    
	String e = request.getParameter("budget");    
	String f = request.getParameter("whatelse");    
	...
	String ds="";
	for(int z=0;d!=null && z<d.length;z++){
    ds+=d[z]+" ";
	}
	...
	int count=st.executeUpdate("insert into photorequest values('"+a+"','"+b+"','"+c+"','"+ds+"','"+e+"','"+f+"','"+g+"','"+h+"','"+i+"','"+j+"')");
	String messg = "Here are your requested quotes:"+'\n';
	if(count!=0)
	{
		
		
	ResultSet rs=st.executeQuery("select * from pro where city='"+h+"' and what='photographer'");
			if(rs.next())
			{
				rs.beforeFirst();
				while(rs.next())
